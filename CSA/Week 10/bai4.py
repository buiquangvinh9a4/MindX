...
#     total_area.append(round((data[data['Region'] == region]['Area'].sum()), 2))

# avg_area = []
# for region in list_region:
#     avg_area.append(round((data[data['Region'] == region]['Area'].mean()), 2))


# new_dataFrame = pd.DataFrame({'Region': list_region, 'Total Area': total_area, 'Average Area': avg_area})
# print(new_dataFrame)
# new_dataFrame.to_excel('D:\Programming\MindX\CSA\Week 10\\data4.xlsx')

max_pop = df['Population'].max()
min_pop = df['Population'].min()

# province/city with max and min population
max_pop_province = df[df['Population'] == max_pop]
min_pop_province = df[df['Population'] == min_pop]

print(max_pop_province.index[0])



# print(max_pop_province.loc[49, 'Name'])
